...
var yadcf = (function ($) {
	...
	function doFilter(arg, table_selector_jq_friendly, column_number, filter_match_mode) {
        ...
        function doFilterSelect(arg, table_selector_jq_friendly, column_number, filter_match_mode) {
...
/* <div class="input-group date date-picker margin-bottom-5" data-date-format="dd/mm/yyyy">
		...
		$(filter_selector_string).append("<div id=\"" + table_selector_jq_friendly + "_date_from_" + column_number + "\" class=\"yadcf-filter-wrapper-inner input-group date date-picker margin-bottom-5 \"  data-date-format=\"dd/mm/yyyy\" >");
                $(filter_selector_string).append("<div id=\"" + table_selector_jq_friendly + "_date_to_" + column_number + "\" class=\"yadcf-filter-wrapper-inner input-group date date-picker \"  data-date-format=\"dd/mm/yyyy\" >");
			
                var filter_selector_string_from = filter_selector_string + " #" + table_selector_jq_friendly + "_date_from_"+column_number;
                var filter_selector_string_to = filter_selector_string + " #" + table_selector_jq_friendly + "_date_to_"+column_number;
		$.fn.dataTableExt.iApiIndex = oTablesIndex[table_selector_jq_friendly];
		oTable = oTables[table_selector_jq_friendly];
		$(filter_selector_string_from).append("<input type=\"text\" placeholder=\"From\" id=\"" + fromId + "\" readonly=\"\" class=\"yadcf-filter form-filter form-control input-sm\" name=\"custom_filter_dateFrom_"+column_number+" \" onkeyup=\"yadcf.rangeDateKeyUP('" + table_selector_jq_friendly + "','" + date_format + "',event);\" \" \>" +
			"</input>");
		$(filter_selector_string_from).append("<span class=\"input-group-btn\"><button class=\"btn btn-sm default\" type=\"button\"><i class=\"fa fa-calendar\"></i></button></span>");
		$(filter_selector_string_to).append("<input type=\"text\" placeholder=\"To\" id=\"" + fromId + "\" readonly=\"\" class=\"yadcf-filter form-filter form-control input-sm\" name=\"custom_filter_dateTo_"+column_number+" \" onkeyup=\"yadcf.rangeDateKeyUP('" + table_selector_jq_friendly + "','" + date_format + "',event);\" \>" +
			"</input>");
		$(filter_selector_string_to).append("<span class=\"input-group-btn\"><button class=\"btn btn-sm default\" type=\"button\"><i class=\"fa fa-calendar\"></i></button></span>");
	...
	function appendFilters(oTable, args, table_selector) {
		...
		for (columnObjKey in args) {
			if (args.hasOwnProperty(columnObjKey)) {
				...
				filter_reset_button_text = false// columnObj.filter_reset_button_text;
				...
				if (filter_container_id === undefined) {
					...
					filter_selector_string = table_selector + " thead tr.filter th:eq(" + col_num_visible + ")";
...
//                                                        console.log(options[ii]);
				...
				if ($filter_selector.length === 1) {
					...
					if (columnObj.filter_type === "select") {
						...
						$(filter_selector_string).append("<select name=\"custom_filter_select_" + column_number + "\"  id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter form-control form-filter input-sm\" " +
                                                "onchange=\"yadcf.doFilterSelect(this, '" + table_selector_jq_friendly + "', " + column_number + ", '" + filter_match_mode + "')\" onclick='yadcf.stopPropagation(event);'>" + options + "</select>");
						...
						$(filter_selector_string).append("<select multiple data-placeholder=\"" + filter_default_label + "\" id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter form-control form-filter input-sm\" " +
						...
						$(filter_selector_string).append("<input id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter form-control form-filter input-sm\" onclick='yadcf.stopPropagation(event);"
						...
						$(filter_selector_string).append("<input type=\"text\" name=\"custom_filter_text_" + column_number + "\" id=\"yadcf-filter-" + table_selector_jq_friendly + "-" + column_number + "\" class=\"yadcf-filter  form-control form-filter input-sm\" onclick='yadcf.stopPropagation(event);"
		...
		if (exFilterColumnQueue.length > 0) {
			(exFilterColumnQueue.shift())();
                ...
                $(".filter-cancel").click(function(){
                $("select.yadcf-filter").val("");
                $("select.yadcf-filter").val("-1");
		resetIApiIndex();
});
	}
	...
	function textKeyUP(table_selector_jq_friendly, event, clear) {
        ...
        function filterCancel(){
            if($('.filter-cancel').click()){
                resetFilters();
            }
        }
        
        function resetFilters(){
            $(".yadcf-filter").val("").focus();
					$(".yadcf-filter" ).removeClass("inuse");
//					oTable.fnFilter("", column_number);
					resetIApiIndex();
        }
	...
	function exFilterColumn(table_arg, col_filter_arr, ajaxSource) {
		...
		if (table_arg.fnSettings().sAjaxSource === null || ajaxSource === true) {
			for (j = 0; j < col_filter_arr.length; j++) {
				...
				case 'text':
				case 'date':
					$('#afafafayadcf-filter-' + table_selector_jq_friendly + '-' + column_number).val(filter_value).addClass('inuse');
	...
	function exGetColumnFilterVal(table_arg, column_number) {
		...
		switch (optionsObj.filter_type) {
		case 'select':
			retVal = $('#fafafyadcf-filter-' + table_selector_jq_friendly + '-' + column_number).val();
			if (retVal === '-1') {
				retVal = '';
			}
			break;
		...
		case 'text':
		case 'date':
			retVal = $('#text_yadcf-filter-' + table_selector_jq_friendly + '-' + column_number).val();
			break;
		case 'multi_select':
