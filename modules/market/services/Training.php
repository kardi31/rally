...
class TrainingService extends Service{
    ...
    public function __construct(){
        $this->trainingTable = parent::getTable('market','training');
        $this->trainingFactorTable = parent::getTable('market','trainingFactor');
    }
    ...
    public function getPerson($id){
        ...
        $q->leftJoin('tf.Market');
        $q->addWhere('tf.market_id = ?',$id);
    ...
    public function checkTodayTrainingForPerson($skill,$market_id){
        $q = $this->trainingTable->createQuery('t');
        $q->addWhere('t.market_id = ?',$market_id);
        $q->addWhere('t.skill_name = ?',$skill);
        $q->addWhere('DATE(t.training_date) = CURDATE()');
        return $q->fetchOne(array(),Doctrine_Core::HYDRATE_ARRAY);
    }
    
    public function getLastSkillTrainingForPerson($skill,$market_id){
        $q = $this->trainingTable->createQuery('t');
        $q->addWhere('t.market_id = ?',$market_id);
        $q->addWhere('t.skill_name = ?',$skill);
    ...
    public function calculateTraining($crews,$rallyService){
        ...
        foreach($crews as $crew){
            ...
            $this->calculateTrainingForPerson($driver['Market'],$crew['km_passed'],$crew['rally_id']);
            
            $pilot = $this->getPerson($crew['pilot_id']);
            $this->calculateTrainingForPerson($pilot['Market'],$crew['km_passed'],$crew['rally_id']);
    ...
    public function calculateTrainingForPerson(Market_Model_Doctrine_Market $person,$km_passed,$rally_id){
        ...
        $trainingResult['market_id'] = $person['id'];
    ...
    public function createRandomTrainingForDriver(Market_Model_Doctrine_Market $driver){
        $driverArray  = array();
        $record = $this->trainingFactorTable->getRecord();
        
        $driverTrainableSkills = $this->driverTrainableSkills;
        
        $driverArray['market_id'] = $driver['id'];
       
        // helper to calculate training blocks
        include(BASE_PATH.'modules/market/services/helper/TrainingBlockDriverHelper.php');
        
        $driverTrainableSkills = $this->driverTrainableSkills;
        
        // helper to calculate training factor
        include(BASE_PATH.'modules/market/services/helper/TrainingFactorDriverHelper.php');
    ...
    public function createRandomTrainingForPilot(Market_Model_Doctrine_Market $pilot){
        $pilotArray  = array();
        $record = $this->trainingFactorTable->getRecord();
        
        $pilotTrainableSkills = $this->pilotTrainableSkills;
        
        $pilotArray['market_id'] = $pilot['id'];
       
        // helper to calculate training blocks
        include(BASE_PATH.'modules/market/services/helper/TrainingBlockPilotHelper.php');
        
        
        $pilotTrainableSkills = $this->pilotTrainableSkills;
        
        // helper to calculate training factor
        include(BASE_PATH.'modules/market/services/helper/TrainingFactorPilotHelper.php');
